...
**Staging update (2026-09-09):** PR #24 merged as `0c73420` and both deployment images are running;
the web applied the harness migrations. The pipeline now has a valid gateway key, with chapter
planning disabled. After paid Pro activation, DeepSeek/DeepInfra, Qwen/Alibaba and Kimi/Alibaba
passed all three small live transport schemas. The probes exposed integer-money accounting and
ID-grounding prompt fixes that must deploy in the follow-up PR. No real-source editorial run or
model-quality audition is complete.
[Exact preflight evidence and follow-up](gateway-staging-qualification-2026-09-09.md).

**Delivery state before the September 9 merge:** PR #23 was merged as `555eb24`, the paired protocol-4 rollout completed, and a 151-minute recording passed one end-to-end transcription run in 11 minutes. [Measured evidence and limits](s2-staging-qualification-2026-09-08.md). PR #24 implements the [replacement design](../pipeline-architecture.md) on a separate branch. The complete recorded-provider chapter browser journey and three structural transcript journeys pass against real local workflows, storage and media. The corrected live checkpointed GPU path passed both short recovery and a 151-minute source. Exact-commit delivery is recorded by the PR's local receipt and GitHub provenance check; the runtime changes had not yet been deployed to shared staging at this checkpoint.
...
| Missing durable model transport | Pinned PydanticAI/Temporal request guard, full response artifacts before validation, qualified immutable route pools, independent verifier policy, bounded reservations and cassettes | Real plugin/replay probe and scoped model-persistence tests pass. Boot validates output/context capacity for every required seat route, including failovers. On September 9 paid Pro activation cleared the initial ZDR refusal. Three families passed all three small transport schemas; integer-money accounting and prompt corrections are in the follow-up PR. Real-source staging runs and model-quality auditions remain pending. |
