...
6. **Unit economics are a feature.** Every metered thing writes to the ledger in the sprint it ships; budgets are breakers, not reports; checkpoints mean a retry re-pays zero tokens.
...
- **Fair-billing rule:** re-runs and quality failures must never feel like paying twice — checkpoints re-pay zero tokens, failed refreshes keep the previous result, and credits are never burned on output the human rejected. `[roadmap]` `[A2]` `[S19]`
