...
| **S2 — Transcript and substrate** | The word timeline exists and the grid over it is proven identical to the legacy's | Transcription is WhisperX on Modal from the first run (Rajesh, 2026-09-05), behind the provider seam with a deterministic mock. The same source has a speaker-labelled, integer-millisecond transcript with revisions, viewable, searchable, and correctable, with speaker rename and SRT/VTT export. The substrate's coarse and fine renderings are byte-identical to the legacy's on three recorded sources; the eval runner reproduces the legacy scorers bit-for-bit through the re-hosted parity chain. Transcription minutes metered. **→ M0** | 6, 8 (substrate, eval runner) |
