...
| **S3 — The harness as a typed, durable program** | Failure is attributable, progress is durable, and no seat has a default vendor | Implement `docs/pipeline-architecture.md`: immutable source evidence and artifact dependencies; separate logical operations and physical attempts; atomic budget reservations before dispatch; unknown outcomes retain exposure and block redispatch; PydanticAI TemporalDurability with request guards inside activities, SDK retries disabled, typed outputs and cassettes in CI. A frozen qualified route snapshot has at least three families per seat including open-weight candidates, and verification excludes every generating/repair family. Gateway probe claims require saved evidence; live transport qualification remains pending that evidence. A joint deterministic compiler produces an exact-cover edit map with shared rational boundaries. Finite run and mutation workflows persist revision-CAS outcomes. Tracing carries metadata without source content. Deliverable: this repository's pipeline architecture and verified implementation | 8 (chassis, economics, LLM seam), 24 (gateway rules) |
...
| Review execution shape | Decided 2026-09-08, PR #24 | Finite run workflow plus finite review workflow per mutation UUID; immutable revisions and CAS own review state |
