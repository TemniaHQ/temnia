...
| **S2 — Transcript and substrate** | The word timeline exists and several segmenters over it are scored against each other, with the legacy's grid as the baseline | Transcription is WhisperX on Modal from the first run (Rajesh, 2026-09-05), behind the provider seam with a deterministic mock. The HLS ladder moves to the same Modal deployment first (NVENC; the VPS laddered a 2.5-hour master at 2.9x realtime, about 46 minutes, in the S1 exit run, and publishing the 3.8 GB ladder to R2 from the VPS took another 21 minutes at eight concurrent puts; Rajesh, 2026-09-06). The publish moves with the ladder. The same source has a speaker-labelled, integer-millisecond transcript with revisions, viewable, searchable, and correctable, with speaker rename and SRT/VTT export. The substrate is designed from the Python ecosystem (Rajesh, 2026-09-07): Segment-any-Text sentences and paragraphs, embedding change-point chapter candidates, one renderer with stable ids and timestamps, behind a segmenter seam with the legacy rules as a byte-parity-tested baseline. The eval runner reports Pk, WindowDiff, GHD, window-tolerant F1 with density, purity and coverage, and tIoU-F1 for every segmenter on the recorded sources, with the legacy scorers re-hosted bit-for-bit as the M1 baseline column; the plan records which segmenter S4 starts on and what number would make us switch. Transcription minutes metered. **→ M0** | 6, 8 (substrate, eval runner) |
| **S3 — The harness as a typed, durable program** | Failure is cheap, progress is durable, and no seat has a default vendor | The editing harness is designed from the Python ecosystem (Rajesh, 2026-09-07), not inherited from the legacy's agent cast: a Temporal workflow per run whose every model call is an idempotency-keyed activity (transcript revision, substrate version, stage, program version, seat, parameters) consulted against a checkpoint table, so the chaos test (a worker killed mid-run resumes with zero repeated model calls) holds by construction; PydanticAI for every model call (typed I/O, JSON-schema outputs validated by our code, TestModel and FunctionModel in CI, OpenTelemetry, Temporal durability); five typed stages, brief, propose in sentence ids never times, cut per boundary in parallel with the substrate's backstops, verify with code first then a different-family 0-1-2 judge and one bounded repair, explain; every stage output a stored, inspectable artefact. Seats are ordered pools from at least three families including open-weight ones, no default vendor, verifier family excluded; every seat goes through one gateway, Vercel AI Gateway as the leading candidate confirmed by the transport probe (full request shape, strict schema, ZDR, cache passthrough) with OpenRouter and a self-hosted LiteLLM proxy as fallbacks. Pure-code failover halt after two independent local-validation failures, per-run budget from the per-source-hour budget checked before every call, ceilings on dispatches, repairs, and revisions; every call in `llm_calls` with seat, models attempted, cache statistics, and cost. Prompts are optimised offline per seat model with DSPy and GEPA against the C2 metrics and the reviewer's reasons, versioned in the registry, promoted only when the golden eval improves; cassettes run in CI, never live models. Tracing through OpenTelemetry into self-hosted Langfuse (metadata-only in production), with the VPS cost measured and Phoenix as the fallback. Amended 2026-09-08 after the S2 review (AGENTS.md; `docs/plans/s3-harness-spec.md`): the substrate is persisted as the first versioned source-evidence artefact; the cut stage is a joint boundary compiler over candidate boundaries, the model's per-boundary answer being one input to its objective; the five typed artefacts are the contract and the calls per stage are measured, not fixed. Deliverable: the pipeline architecture document for this repository | 8 (chassis, economics, LLM seam), 24 (gateway rules) |
| **S4 — The chapter lane** | The coverage lane runs end to end on recorded sources, with no UI | Entry gate (2026-09-08): acceptable boundary windows annotated on the three recordings exist before the cut objective is tuned. From a recorded source, brief → propose (an exact cover in sentence ids, validated in code) → cut (per boundary, with the substrate's backstops) → verify (code validators, then a different-family judge, one bounded repair) → explain → the plan persisted → render → file sensors produce chapter files and `chapters.json` that pass the full battery, under budget, with window-tolerant boundary F1 and tIoU-F1 at or above the legacy baseline as the C2 runner reports them, the per-seat audition recorded (open-weight models first-class, cost per correct the decider), and dollars per source-hour recorded on the run. Deliverable: the clip-cut architecture document for this repository | 9 (segments), 5 (program cuts), 12 (file sensors) |
