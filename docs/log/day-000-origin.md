...
For a while now I've had a question I can't answer by reading other people's posts about it: how far, and how fast, can one experienced engineer go with AI coding agents? Not a demo, agents can make a demo. Something a business would trust with its client work, with proper security, an architecture that survives growth, and a person accountable for every line. The kind of thing I've spent the last 13 years building the slow way.

So I'm going to try, in the open.

I want to be clear about what this is and isn't, because I know how it sounds. It isn't vibe coding. I'll decide the architecture, the data model, where the security boundaries sit, and what has to pass before anything merges. Claude Code, Codex and Cursor will write most of the code. I'll read all of it. If bad code gets through, that's my failure, not the tool's. That applies to these posts too: the agents draft them from my daily log and I edit every one before it goes out. A tech lead with a team of agents, and the question is whether that holds up six months in, when I can no longer keep the whole codebase in my head and the agents are changing systems they didn't write.

What I'm building is called Temnia. If you run a content agency, you know the shape of the problem. A client sends over a two-hour podcast, and someone on your team spends the afternoon cutting it into chapters by hand, because that's what really goes out: the chaptered episode, the segments, the timestamps, the show notes, the mid-roll points. Then the client approves version three and version four ships. Temnia cuts the whole episode into chapters that are right, hangs everything else off that spine, and gives every piece a path back to the exact seconds it came from, so you can check a clip actually says what its caption claims. Nothing publishes because a model felt confident. A person approves one specific version.

The part I care most about is under the hood. Models are good at judgment and bad at precision, and a chapter cut punishes imprecision: every second has to land in exactly one chapter or be a deliberate drop, and a wrong boundary breaks the chapters on both sides of it. So a model gets to say what matters, and code gets to say where the cut lands, checks it, and keeps a record. Every proposal stays a proposal until a person accepts it. Models will keep getting swapped for better ones. The system around them is what has to last.

Why this idea? Two reasons, and the first is that it's hard, which is the point. Ten-gigabyte uploads that have to resume, transcodes that run for hours, transcripts aligned to the word, a two-hour timeline that doesn't stutter in a browser, every client's footage walled off from every other's. All of it fails quietly. I've already had a transcoder treat a dropped connection as the end of the file and report success. That's the kind of problem I want to watch agents handle with a real engineer setting the bar. A todo app wouldn't tell me anything.

The second is the gap. Every AI video tool is playing the same game, mining the episode for thirty-second highlights, and a highlight is forgiving. Start it early, end it late, nobody checks what you left out. The episode itself, cut properly, is still an editor's afternoon, and the few tools that try it are built for one editor with a black box in the middle. Nobody is building that for an agency with ten clients and a sign-off on every asset. That's the gap.

One post a day from here. What shipped, what broke, where the agents helped and where they wasted my time, and real numbers with dates once I have them. Day 1 is tomorrow.

If you cut episodes for clients every week, I'd genuinely like to know which part I'm underestimating. That answer shapes what gets built first.
