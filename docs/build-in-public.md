...
- The log is committed to the repo. It is the source of truth for the post, and later it is the project's own history.
...
2. **Handles.** Reserve the X handle and the LinkedIn company page — `temnia`, then `temniahq`, then `temniaai`, the same string on both platforms. The GitHub org is `TemniaAI` (github.com/TemniaAI/temnia).
