...
retaining the full package despite its [unpatched upstream advisory](https://github.com/nltk/nltk/security/advisories/GHSA-8mgp-746c-j5xp)
when Temnia used only these functions. The frozen oracle made 18,228 comparisons with
maximum difference 0.0. This does not claim that a Temnia exploit was reproduced; attribution,
license and source hashes are in [the third-party notice](../apps/pipeline/THIRD_PARTY_NOTICES.md).
...
Twenty-six slots differ from the legacy stack. Rows 1–3 were decided with the runtime; rows 18 and 28 were decided by Rajesh on review; row 8 records a proposal he reversed; row 26 was added at S3; the rest were accepted with the document on 2026-09-05.
...
| 9 | Model gateway | OpenRouter, fixed | **None on the primary path** (2026-09-15): the harness calls Anthropic, OpenAI and Google directly through PydanticAI's adapters with the SDKs' retries off, prices from the route table with the usage in each response, and paces from the vendors' rate-limit headers. A gateway returns only as Vercel AI Gateway with BYOK for an open-weight lane if a single key is ever wanted again | The OpenRouter route pool of the 2026-09-07 vendor rules and OpenRouter's lookup-based billing were the causes of the September staging stops (three quota-bound hops; a charge known minutes later, hence receipts, fences and a reaper). A direct vendor returns usage in the response and its quota is ours. Decision and design: AGENTS.md 2026-09-15, `docs/design/topics-vendors-and-usage-2026-09-15.md`. PR24's gateway seam stays in the tree for the legacy programs until the deletion step |
...
| 25 | Agent framework | Mastra + AI SDK | **PydanticAI 2.x (MIT)** on Temporal via its `TemporalDurability` capability; no framework owns control flow | Decided at S3 (Rajesh, 2026-09-07). PydanticAI types every model call without owning control flow, so the 2026-09-05 objection does not apply; compared against the OpenAI Agents SDK + Temporal (OpenAI-only integration), the Claude Agent SDK (a coding harness, provider-native), LangGraph (its own durable runtime beside Temporal), and DSPy at runtime (hides the prompt structure caching depends on). Basis: the MAST 2025 failure taxonomy, compute-controlled single-vs-multi-agent results, and domain evidence from EditDuet, Crayotter, CineAgents, PODTILE, and Chapter-Llama |
| 26 | Prompt optimisation | not a legacy slot | **DSPy 3.3.1 (MIT) with GEPA**, offline, per seat model | New at S3 (Rajesh, 2026-09-07). Metric is the C2 segmentation metrics plus the reviewer's reasons as textual feedback; artefacts are versioned by seat and model in the prompt registry and promoted only when the golden eval improves; cassettes run in CI, never live models |
| 27 | Dates | date-fns + @internationalized/date | **Decide at S30** | The Temporal date API reached Stage 4 in March 2026 and ships in Chrome 144+, Firefox 139+, Node 26; Safari partial |
| 28 | Hosted transcription adapters | Deepgram and AssemblyAI at S3 | **Fallback only, built when needed** | With WhisperX primary, a hosted adapter exists for outage cover or throughput bursts; AssemblyAI Universal-3.5 Pro is the candidate on its diarization numbers |
...
| Dates | decide at S30 | Temporal API + polyfill, date-fns 4, @internationalized/date | §0 row 27 |
...
| Browser uploader | **Uppy 6.0.0** Dashboard + `@uppy/aws-s3` in `signRequest` mode; Complete and Abort are app routes | in-house client (S1, removed), tus (needs a service) | §14 2026-09-07 |
...
| Transcription | **WhisperX 3.8.6** (large-v3, wav2vec2 alignment, pyannote diarization) on **Modal**, behind the provider seam with a deterministic mock | AssemblyAI Universal-3.5 Pro, Deepgram Nova-3, ElevenLabs Scribe, Gladia, Parakeet | Decided (§0 row 18). Word timings and speaker ids land in the same canonical transcript JSON; the adapter contract is unchanged, so a hosted fallback slots in without touching the substrate |
...
| Slot | Pick | Compared | Verdict |
|---|---|---|---|
| Gateway | **Direct vendor APIs** (Anthropic, OpenAI, Google) through PydanticAI's own adapters; no gateway on the primary path (2026-09-15) | Vercel AI Gateway with BYOK for an open-weight lane (zero markup, usage per response); not OpenRouter | Usage in the response, our own quota, official retries off in favour of the harness ladder, automatic prompt caching on Anthropic, structured outputs on all three. Zero data retention is a per-account grant requested from each vendor and recorded in the snapshot's account terms; the boot log names routes still waiting. See `docs/design/topics-vendors-and-usage-2026-09-15.md` |
...
| Models | **Frontier first** (Rajesh, 2026-09-15, reversing the 2026-09-05 no-default-vendor rule for the primary tier): author and repair **Claude Opus 5** ($5/$25 per MTok), reviewer **GPT-5.6 Terra** ($2/$12) from a family independent of the author, inventory **Claude Sonnet 5** ($2/$10); fallbacks GPT-5.6 Sol ($4/$20) for the author and Gemini 3.8 Flash ($0.75/$3.75) for review and inventory. A production seat pool needs two families; the verifier is always a different family from the generator (PRD §1.5 rule 8) | Open-weight models (Kimi K3, DeepSeek V4, GLM-5.3, Qwen 3.8, MiniMax M3) as a per-seat audition on cost-per-correct against the frontier baseline, once three held-out recordings finish without a code-caused stop | The seats come straight from the 2026-08-26 audition evidence (Opus top-tier in both clip passes, Terra the best drop taxonomy at a tenth of the price, Sonnet the high-volume workhorse; Gemini's earlier DNQ was a schema issue since cured). Fable 5.1 ($10/$50) stays excluded: not served under zero data retention without express authorisation. Frontier-first is development spend bounded by the per-run allowance |
...
| Structured output | PydanticAI `output_type` (native JSON Schema where the seat supports it, tool-based elsewhere) + local pydantic validation by our own code + one bounded repair | Instructor, hand-rolled JSON parsing | Decided with the S3 harness design (2026-09-07); validation never trusts the provider's success |
...
| Tracing and prompt registry | **Logfire** (PydanticAI's own instrumentation plus its Temporal plugin): one trace per run, one span per activity and model call, tokens on the span, prompts and completions off (`include_content=False`); on only when `LOGFIRE_TOKEN` is set on the pipeline service (2026-09-15) | Self-hosted Langfuse v4 (the S3 decision; needs Postgres, ClickHouse, Redis and object storage), Phoenix | Logfire is the instrumentation the framework and the Temporal plugin already emit; nothing to host, and the free tier covers development. Revisit when the metadata-only production rule and a self-host cost are both measured |
...
- The per-seat audition result decides seats; a new open-weight release with JSON-schema outputs and a million-token context is an audition trigger.
...
| 2026-09-06 (S1) | Uploader was **in-house** (server-signed part URLs, server-side control calls and resume); Uppy 6.0 rejected | Uppy 6 (2026-08-26) rewrote `@uppy/aws-s3` to do Create/ListParts/Complete/Abort from the browser on presigned URLs, and the research said R2 cannot serve those. That claim was wrong (probed 2026-09-07); superseded by the row below |
