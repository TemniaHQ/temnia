...
| 2026-09-06 (S1) | Uploader is **in-house** (server-signed part URLs, server-side control calls and resume); Uppy 6.0 rejected | Uppy 6 (2026-08-26) rewrote `@uppy/aws-s3` to do Create/ListParts/Complete/Abort from the browser on presigned URLs. The S1 research said R2 cannot serve those; a probe against the staging bucket on 2026-09-07 showed it can (all four presigned operations succeed; R2's exclusion is HTML-form POST policies). The choice stands on server-side control calls and fingerprint resume, an ~250-line client, and no dependency on an eleven-day-old rewrite; tus needs another service |
