...
| 9 | Model gateway | OpenRouter, fixed | **Vercel AI Gateway**, current transport candidate; live qualification artifact required. OpenRouter and a self-hosted LiteLLM proxy remain alternatives | PR24 implements the OpenAI-compatible strict-schema/ZDR route seam, explicit provider routing and reported-cost reconciliation. The September7 probe claim had no saved artifact in the repository; September8 catalogue discovery returned372 models, while authenticated request/privacy/cache qualification remains pending. A catalogue entry is not a qualified route. See `docs/pipeline-architecture.md` |
...
| Slot | Pick | Compared | Verdict |
|---|---|---|---|
| Gateway | **Vercel AI Gateway** is the implemented candidate transport, pending saved authenticated strict-schema/ZDR and provider-specific qualification | OpenRouter, a self-hosted LiteLLM proxy and other qualified gateways may compete through the seam | PR24 uses the OpenAI-compatible endpoint with SDK retries disabled, explicit provider routing, response artifacts and generation-cost reconciliation. Native Anthropic transport and caching are not assumed qualified; enabling either requires a saved request-shape probe. No production model route is selected from the public catalogue alone. See `docs/pipeline-architecture.md` |
...
| Models | **No default vendor** (Rajesh, 2026-09-05); open-weight models are first-class in every seat's pool, not a fallback tier. Every seat gets an ordered pool drawn from at least three model families, probed for transport at S3 and auditioned per seat at S4 on cost-per-correct; the verifier seat is always a different family from the generator (PRD §1.5 rule 8), so production runs at least two vendors by construction. Current-generation candidates on the research date: Moonshot **Kimi K3** (modified-MIT weights, 1M context, JSON-schema outputs, $3/$15 per MTok, $0.30 cache hits), DeepSeek V4 Pro and V4 Flash, Zhipu GLM-5.3 and GLM-5.3-Flash, Alibaba Qwen 3.8 Max and Flash, MiniMax M3, beside Anthropic Claude Opus 5 ($5/$25 per MTok) and Sonnet 5 ($2/$10), OpenAI GPT-5.6 Sol ($5/$30), Terra ($2/$12), and Luna ($0.20/$1.20), Google Gemini 3.8 Flash and the 3.x Pro line, and xAI Grok 4.5 | Claude Fable 5.1 ($10/$50) | Fable 5.1 is excluded by policy, not preference: it is not served under zero data retention without express authorisation. The legacy's August audition, in which the then-current Opus won both clip passes at $0.316 a run with GPT-5.6 Terra as the cheap challenger and Kimi K3 best on grounding, is evidence about a field that has since moved; it is re-run, not inherited. Every seat is decided per seat on cost-per-correct and the result is recorded, never inherited. The gateway's models endpoint is the live catalogue; this list is the audition's starting roster, never a routing table |
