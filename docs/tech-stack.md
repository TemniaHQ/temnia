...
| 9 | Model gateway | OpenRouter, fixed | **Vercel AI Gateway**, confirmed by the S3 transport probe (2026-09-07); OpenRouter and a self-hosted LiteLLM proxy as fallbacks | Vercel: zero token markup including BYOK, per-request ZDR free on Pro, both OpenAI-compatible and Anthropic Messages-compatible endpoints, cache_control passthrough and caching: auto, and image and video model types in the catalogue. OpenRouter: 5.5% credit fee, free ZDR, three documented outages in eight months, no SLA. The probe confirmed the full request shape, strict schema, ZDR, and cache passthrough, and checked the reported one-hour-cache downgrade. Catalogue coverage in §9 |
...
| Slot | Pick | Compared | Verdict |
|---|---|---|---|
| Gateway | **Vercel AI Gateway**, confirmed by the S3 transport probe (Rajesh, 2026-09-07): hundreds of models, zero markup including BYOK, routing rules, fallbacks, budgets, team-level ZDR routing, no-training controls, OpenAI-compatible and Anthropic-Messages surfaces, `cache_control` passthrough and `caching: auto` | OpenRouter, a self-hosted LiteLLM proxy (per-key budgets), Portkey (acquired by Palo Alto Networks, May 2026); both kept as fallbacks | §0 row 9. The probe confirmed the full request shape, strict schema, ZDR, and cache passthrough, and checked the reported one-hour-cache downgrade. Every seat goes through the gateway; the harness depends only on the unified surface plus JSON-schema outputs validated by our code. Vercel's Anthropic Messages-compatible endpoint lets the official `anthropic` SDK be used for Claude seats |
