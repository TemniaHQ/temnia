...
for (const policy of TOPIC_POLICIES) {
  test(`${policy} generation, human correction and exact accepted exports`, async ({
    page,
  }) => {
    ...
    if (policy === TOPIC_POLICY) {
      ...
      expect(
        assessment.candidates[0]?.coldReview?.completeDiscussion.status
      ).toBe("unknown");
    ...
    await page.getByRole("tab", { exact: true, name: "Topic videos" }).click();
    await expect(
      panel.getByText("Accepted by you", { exact: true })
    ).toBeVisible({ timeout: STAGE_TIMEOUT });
    await panel
    ...
    expect(correctedChild.renders[0]?.media.id).toBe(
      child.renders[0]?.media.id
    );
    ...
    if (policy === TOPIC_SELECTION_POLICY) {
      ...
      expect(assessment.coldReviews).toEqual([]);
      expect(assessment.portfolioReview).toBeNull();
      expect(assessment.proposerFamily).toBe("human");
      expect(assessment.executionStatus).toBe("needs_review");
    }
    await expect(
    ...
    const chapters = await page.request.get(
